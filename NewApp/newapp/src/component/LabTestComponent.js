...
function App() {
  ...
  const handleGetTestResults = async (event) => {
    ...
    try {
      const response = await axios.get(`/labtests/${testId}/results`);
    ...
    <div>
      ...
      <ul>
        {labTests.map((labTest) => (
          <li key={labTest.testId}>
            Test ID: {labTest.testId}, Patient ID: {labTest.patientId}, Status: {labTest.status}
          </li>
        ))}
      </ul>
      ...
      <ul>
        {labTestResults.map((labTestResult) => (
          <li key={labTestResult.testId}>
            Test ID: {labTestResult.testId}, Result Data: {labTestResult.resultData}, Result Date: {labTestResult.resultDate}
          </li>
        ))}
      </ul>
